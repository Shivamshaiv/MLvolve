...
class Funding(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.model = model
        self.fund_no = len(
            [agent for agent in model.schedule.agents if agent.category == "fund"]
        )
        self.category = "fund"
        self.unique_id = "fund_" + str(self.fund_no)
        self.grant_money = pyro.sample(
            self.namegen("funding_money"), pyd.Uniform(50000, 20_00000)
        ).item()
    ...
    def allocate_funding(self):
        other_funds = [
            agent for agent in self.model.schedule.agents if (agent.category == "fund")
        ]
        ...
        seniors_agents = [
            agent for agent in self.model.schedule.agents if (agent.category == "S")
        ]
        ...
        for agent in sorted_seniors[sum_till:]:
            if cap > 0:
                ...
                agent.landscape.bid_win_sig_store.append(
                    agent.landscape.matrix[agent.pos_y, agent.pos_x]
                )
...
class Recruiter(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.model = model
        self.rec_no = len(
            [agent for agent in model.schedule.agents if agent.category == "rec"]
        )
        self.category = "rec"
    ...
    def allocate_funding(self):
        ...
        for fund in sorted_funds:
            seniors_agents = [
                ...
                if (agent.category == "S" and not agent.is_funded)
            ]
            pref = self.pref_generator()
            sorted_seniors = sorted(
                seniors_agents,
                key=lambda x: (
                    pref[0] * x.current_bid
                    + pref[1] * x.bid_novelty
                    + pref[2] * x.reputation_points
                )
                / sum(pref),
                reverse=True,
            )
