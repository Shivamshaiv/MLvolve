...
class Labs(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.model = model
        self.unique_id = "Lab_" + str(
            len([agent for agent in model.schedule.agents if agent.category == "lab"])
            + 1
        )
        ...
        self.capacity_s = 0
        self.init_seniors(
            int(pyro.sample(self.namegen("capcity_j"), pyd.Poisson(3)).item() + 1)
        )
    ...
    def gen_lab_repute(self):
        senior_reputation = [
            agent.reputation_points
            for agent in self.model.schedule.agents
            if (agent.category == "S" and agent.affliation == self.unique_id)
        ]
        sorted_repu = sorted(senior_reputation)
        if len(sorted_repu) <= 1:
            return np.mean(sorted_repu)
        return (sorted_repu[-1] + sorted_repu[-2] + sum(sorted_repu[:-2])) / (
            2 + (0.5 * len(sorted_repu[:-2]))
        )
  ...
  def recruit_juniors(self):
    ...
    def recruit_students(self):
        ...
        other_labs = [
            agent for agent in self.model.schedule.agents if (agent.category == "lab")
        ]
        sorted_labs = sorted(other_labs, key=lambda x: x.lab_repute, reverse=True)
        own_position = sorted_labs.index(self)
        self.lab_rank = own_position
        ...
        student_agents = [
            agent for agent in self.model.schedule.agents if (agent.category == "U")
        ]

        sorted_student = sorted(
            student_agents, key=lambda x: x.reputation_points, reverse=True
        )
        # self.print_agent_list(sorted_juniors)
        # self.print_agent_list(sorted_labs)
        sum_till = int(sum(sorted_lab_capacity[:own_position]))
    ...
    def step_stage_2(self):
        # self.lab_repute = self.gen_lab_repute()
        seniors_recurited_list = [
            agent
            for agent in self.model.schedule.agents
            if (agent.category == "S" and agent.affliation == self.unique_id)
        ]
        for senior in seniors_recurited_list:
            senior.lab_coupled_reputation = (senior.reputation_points * (0.5)) + (
                self.lab_repute * (0.5)
            )
    ...
    def printing_step(self):
        print(
            "Reputation Rank of lab is",
            self.lab_rank,
            "Vacancy in",
            self.unique_id,
            " is of students:",
            self.capacity_s,
            self.capacity_s_initial,
        )
