...
from utils.landscape_utils import (add_gaussian, bivariate_normal, get_peak,
                                   remove_gussian_peak, top_k_2d_array)
...
class Episthemic_Landscape(Agent):
    ...
    def __init__(self, unique_id, model, size=100, number_of_peaks=40):
        ...
        self.model = model
        self.topic = len(
            [agent for agent in model.schedule.agents if agent.category == "Elandscape"]
        )
        ...
        self.impact_matrix = np.random.rand(self.size, self.size)
        ...
        self.frame2 = go.Heatmap(
            z=np.where(self.bid_store == 1, 1, None),
    ...
    def simulate_guassian_peaks(self, number, width=7, height=3):
        for _ in range(number):
            ...
            self.matrix = add_gaussian(
                self.matrix, self.size, [locx, locy], sig, new_height, self.max_height
            )
            self.diff_matrix = add_gaussian(
                self.diff_matrix,
                self.size,
                [locx, locy],
                sig,
            ...
            sig = width * np.random.normal(0.8, 0.2)
            new_height = height * np.random.normal(1, 0.2)
            self.impact_matrix = add_gaussian(
                self.impact_matrix,
                self.size,
                [locx, locy],
                sig,
    ...
    def step_stage_final(self):
        ...
        self.best_bid_store.append(
            sum(top_k_2d_array(self.original_mat, len(self.bid_win_sig_store)))
        )
