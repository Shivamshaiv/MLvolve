...
class Student(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.model = model
        self.unique_id = "U_" + str(
            len([agent for agent in model.schedule.agents if agent.category == "U"]) + 1
        )
    ...
    def age_gen(self):
        age = pyro.sample(self.namegen("age_"), pyd.Uniform(18, 21))
        return int(age.item())
    ...
    def gen_publications(self):
        # print("Pub given")
        num_pub = pyro.sample(
            self.namegen("number_publication"), pyd.Poisson(self.iq / 100.0)
        )
        return num_pub.item()
    ...
    def gen_citations(self):
        ...
        h_index = (
            h_index_computed.item()
            + pyro.sample(self.namegen("h_index_adding"), pyd.Uniform(0.5, 1)).item()
        )
        if h_index < 0:
            h_index = 0
    ...
    def gen_reputation_points(self, model):
      ...
      """
        ...
        w_publications = 1.2
        w_iq = pyro.sample(
            self.namegen("how_iq_matters"), pyd.Uniform(0.75, 1.5)
        ).item()
        ...
        rep_points = (
            w_citations * (self.citations / max(cit_list))
            + w_publications * (self.publications / max(pub_list))
            + w_iq * (self.iq / max(iq_list))
        ) / (w_citations + w_publications + w_iq)
        return rep_points
...
class Junior(Agent):
    ...
    def __init__(self, unique_id, model, promoted_attrs=None):
        super().__init__(unique_id, model)
        self.unique_id = "J_" + str(
            len([agent for agent in model.schedule.agents if agent.category == "J"]) + 1
        )
    ...
    def age_gen(self):
        age = pyro.sample(self.namegen("age_"), pyd.Uniform(22, 28))
        return int(age.item())
    ...
    def gen_reputation_points(self, model):
      ...
      """
        ...
        w_publications = 1.2
        w_iq = pyro.sample(
            self.namegen("how_iq_matters"), pyd.Uniform(0.2, 0.45)
        ).item()
    ...
    def promote_to_senior(self):

        promoted_attrs = dict()
        promoted_attrs["iq"] = self.iq
        promoted_attrs["location"] = self.location
        promoted_attrs["publications"] = self.publications
        promoted_attrs["citations"] = self.citations
        promoted_attrs["ambitions"] = self.ambitions
        promoted_attrs["reputation_points"] = self.reputation_points
        promoted_attrs["age"] = self.age
        senior_aff = [
            agent
            for agent in self.model.schedule.agents
            if (agent.unique_id == self.affliation)
        ][0]
        ...
        self.model.schedule.add(
            Senior(
                0,
                self.model,
                affliation=senior_aff.affliation,
                promoted_attrs=promoted_attrs,
            )
        )
        print("Promoted with affliation", senior_aff.affliation)
        self.model.schedule.remove(self)

    def select_topic(self):
        k = self.numtopics = 5
        return pyro.sample(
            self.namegen("topic_select"), pyd.Categorical(torch.tensor([1 / k] * k))
        ).item()
...
class Senior(Agent):
    ...
    def __init__(self, unique_id, model, affliation=None, promoted_attrs=None):
        super().__init__(unique_id, model)
        self.unique_id = "S_" + str(
            len([agent for agent in model.schedule.agents if agent.category == "S"]) + 1
        )
        self.category = "S"  # Senior Researcher Category
        self.numtopics = 5
        self.topic_interested = self.select_topic()  # We assume that there are 5 topics
        self.ambitions, self.novelty_prefrence = self.goal_generator()
        self.novelty_prefrence = self.ambitions[0]  # This is just an experiment.
    ...
    def age_gen(self):
        age = pyro.sample(self.namegen("age_"), pyd.Uniform(29, 69))
        return int(age.item())
    ...
    def set_epithemic_landscape(self):
        return [
            agent
            for agent in self.model.schedule.agents
            if (agent.category == "Elandscape" and agent.topic == self.topic_interested)
        ][
            0
        ]  # Because the array will only have one element which is THE Landscape
    ...
    def make_funding_bid(self):
        ...
        dist_mat = np.zeros([max_size, max_size])
        if self.model.timestep > 1:
            for x, y in zip(self.landscape.visible_x, self.landscape.visible_y):
                if (x == self.pos_y and y == self.pos_x) or (
                    round(
                        self.landscape.matrix[self.pos_y, self.pos_x]
                        / self.landscape.max_height,
                        2,
                    )
                    == 0
                ):
                    continue
                temp_mat[y, x] = (
                    (w[2] * self.landscape.matrix[y, x] / self.landscape.max_height)
                    + (
                        w[1]
                        * self.modifiers["difficulty"]
                        * (self.landscape.diff_matrix[y, x] / self.landscape.max_height)
                    )
                    - (
                        self.novelty_prefrence
                        * self.modifiers["novelty"]
                        * self.l2_dist(y, x)
                    )
                )
                dist_mat[y, x] = self.l2_dist(y, x)
            self.search_history.append(temp_mat)

            # Episilon greedy
            if (
                pyro.sample(self.namegen("e_greedy"), pyd.Uniform(0, 1)).item()
                < self.model.episilon
            ):
                ...
                # print("The position is",(self.pos_y,self.pos_x),"We go to",(max_index[0],max_index[1]),"the dist is",dist_temp)
        ...
        self.current_bid = (
            self.landscape.matrix[self.pos_y, self.pos_x] / self.landscape.max_height
        )
        self.difficulty_selected = self.landscape.diff_matrix[self.pos_y, self.pos_x]
        self.bid_novelty = self.compute_novelty()
        self.bid_history.append(
            (
                round(self.current_bid, 2),
                round(self.bid_novelty, 2),
                round(self.reputation_points, 2),
                round(dist_temp, 2),
            )
        )
        # self.trajectory.append([self.pos_x,self.pos_y])
        ...
        self.bid_value = (
            w1 * self.current_bid + w2 * self.bid_novelty + w3 * self.reputation_points
        ) / (w1 + w2 + w3)
    ...
    def compute_novelty(self):
        if self.model.timestep <= 1:
            ...
            invert = True
            if invert:
                discovered_inv = np.abs(
                    1 - self.landscape.bid_store
                )  # Inverting Discovery for novelty
            else:
    ...
    def compute_impact(self):
        significance = self.current_bid
        novelty = self.bid_novelty
        raw_impact = self.landscape.impact_matrix[self.pos_y, self.pos_x]
        for key in ["ME", "me", "them", "THEM"]:
            # print(np.max([self.model.i_var[key],0]))
            benifits = (
                significance * float(np.max([self.model.i_var[key], 0])) * raw_impact
            )
            risks = (
                significance
                * novelty
                * float(np.abs(np.min([self.model.i_var[key], 0])))
                * raw_impact
            )
            impact = float(benifits) + float(risks)
            self.model.impact[key] += impact

    def recruit_juniors(self):
        other_seniors = [
            agent
            for agent in self.model.schedule.agents
            if (agent.category == "S" and agent.is_funded)
        ]
        sorted_seniors = sorted(
            other_seniors, key=lambda x: x.lab_coupled_reputation, reverse=True
        )
        sorted_seniors_capacity = [agent.capacity_j_initial for agent in sorted_seniors]
        own_position = sorted_seniors.index(self)
        self.sorted_seniors = (
            sorted_seniors  # Stored to in the variabe to remove reduntant computations
        )
        self.senior_rank = own_position
        juniors_agents = [
            agent for agent in self.model.schedule.agents if (agent.category == "J")
        ]

        sorted_juniors = sorted(
            juniors_agents, key=lambda x: x.reputation_points, reverse=True
        )
    ...
    def compute_optimal_vacancies(self):
        ...
        own_lab = [
            agent
            for agent in self.model.schedule.agents
            if (agent.category == "lab" and agent.unique_id == self.affliation)
        ][
            0
        ]  # Only one element of array
    ...
    def work_on_projects(self):
        ...
        lab_students = [
            agent
            for agent in self.model.schedule.agents
            if (agent.category == "U" and agent.affliation == self.affliation)
        ]
        rep_u = []
        u_agents = []
        for agent in lab_students:
            if not agent.booked:
                if (
                    self.own_lab_agent.capacity_s_initial
                    >= self.own_lab_agent.capacity_s
                ):
        ...
        j_agents = [
            agent
            for agent in self.model.schedule.agents
            if (agent.category == "J" and agent.affliation == self.unique_id)
        ]
        rep_j = [agent.reputation_points for agent in j_agents]
        self.project_productivity = (
            productivity(rep_j, rep_u, self.compute_productivity)
            * self.reputation_points
        )  # This includes all the agents and compute
        self.pub_productivity = (sum(rep_j) + sum(rep_u)) + self.compute_productivity
        d = self.difficulty_selected
        chance = self.chance_of_project_success()
        self.is_successful = pyro.sample(
            self.namegen("Proj_success"), pyd.Bernoulli(chance)
        ).item()
        if self.is_successful == 1:
            print(
                "Chance of project by",
                self.unique_id,
                "is",
                round(chance, 5),
                "with difficulty",
                round(d, 3),
                "with bid of",
                round(self.bid_value, 4),
                "topic was",
                self.topic_interested,
                ".....it was a SUCCESS",
            )
            vision = 4
            ...
            publication_generated = (
                pyro.sample(
                    self.namegen("publication_gen"), pyd.Poisson(self.pub_productivity)
                ).item()
                + 1
            )
            citations_generated = (
                publication_generated
                * pyro.sample(
                    self.namegen("citation_gen"),
                    pyd.Poisson(self.landscape.matrix[self.pos_y, self.pos_x]),
                ).item()
            )  # Significance result to citations
            ...
            publication_generated = (
                pyro.sample(
                    self.namegen("publication_gen"),
                    pyd.Poisson(self.pub_productivity * chance),
                ).item()
                + 1
            )
            citations_generated = (
                ...
                * pyro.sample(
                    self.namegen("citation_gen"),
                    pyd.Poisson(self.landscape.matrix[self.pos_y, self.pos_x] * chance),
                ).item()
            )
        ...
        for xi in range(-vision, vision + 1):
            for yi in range(-vision, vision + 1):
                if xi ** 2 + yi ** 2 <= (vision + 0.5) ** 2:
                    ...
                    self.landscape.explored[
                        (self.pos_y + yi) % max_size, (self.pos_x + xi) % max_size
                    ] = 1
    ...
    def gen_reputation_points(self, model):
      ...
      """
        ...
        w_publications = 1.1
        w_iq = pyro.sample(
            self.namegen("how_iq_matters"), pyd.Uniform(0.0, 0.0001)
        ).item()
    ...
    def printing_step(self):
        ...
        if self.is_funded:
            print(
                ...
                "with a bid of",
                self.current_bid,
                "and recruitment of juniors happened as",
                (self.capacity_j, self.capacity_j_initial),
            )
        else:
            print(
                self.unique_id,
