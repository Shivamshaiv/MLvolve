...
class Estimator:
    ...
    def __init__(
        self, sess, action_dim, state_dim, env, scope="estimator", summaries_dir=None
    ):
        self.sess = sess
        ...
        with tf.variable_scope(scope):
            ...
            self.loss_gradients = tf.gradients(
                self.loss, tf.trainable_variables(scope=scope)
            )
        ...
        self.summaries = tf.summary.merge(
            [
                ...
                tf.summary.scalar(
                    ...
                    tf.reduce_sum([tf.norm(item) for item in self.loss_gradients]),
                ),
            ]
        )
        ...
        for idx, node_id in enumerate(env.target_grids):
            neighbor_indices = env.nodes[node_id].layers_neighbors_id[
                0
            ]  # index in env.nodes
            neighbor_ids = [
                env.target_grids.index(env.nodes[item].get_node_index())
        ...
        self.valid_neighbor_node_id = np.zeros(
            (self.n_valid_grid, self.action_dim)
        )  # id in env.nodes
        self.valid_neighbor_grid_id = np.zeros(
            (self.n_valid_grid, self.action_dim)
        )  # id in env.target_grids
        for grid_idx, grid_id in enumerate(env.target_grids):
            for neighbor_idx, neighbor in enumerate(self.env.nodes[grid_id].neighbors):
                if neighbor is None:
                    ...
                    self.valid_neighbor_grid_id[
                        grid_idx, neighbor_idx
                    ] = env.target_grids.index(node_index)
    ...
    def _build_model(self):
        trainable = True
        self.state = X = tf.placeholder(
            shape=[None, self.state_dim], dtype=tf.float32, name="X"
        )
    ...
    def action(self, s, context, epsilon):
        ...
        for idx, grid_valid_idx in enumerate(grid_ids):
            ...
            if np.sum(valid_qvalues) == 0:
                ...
                temp_qvalue[self.valid_action_mask[grid_valid_idx] > 0] = 1.0 / np.sum(
            ...
            for curr_action_idx, num_driver in enumerate(curr_action_indices):
                if num_driver > 0:
                    end_node_id = int(
                        self.valid_neighbor_node_id[grid_valid_idx, curr_action_idx]
                    )
    ...
    def compute_targets(self, valid_prob, next_state, node_reward, gamma):
        ...
        qvalue_next = self.sess.run(
            self.value_output, {self.state: next_state}
        ).flatten()  # qvalue of next state
        ...
        for idx in np.arange(self.n_valid_grid):
            ...
            curr_grid_target = (
                node_reward[neighbor_grid_ids][best_grid]
                + gamma * qvalue_next[neighbor_grid_ids][best_grid]
            )
    ...
    def _build_cnn_model(self):
        ...
        self.state_spacetime = Xst = tf.placeholder(
            shape=[None, self.n_valid_grid + self.T], dtype=tf.uint8, name="Xst"
        )

        # states of distribution
        self.state = X = tf.placeholder(
            shape=[None, self.M, self.N, 4], dtype=tf.uint8, name="X"
        )
...
class stateProcessor:
    ...
    def __init__(self, target_id_states, target_grids, n_valid_grids):
        self.target_id_states = (
            target_id_states  # valid grid index for driver and order distribution.
        )
    ...
    def to_grid_states(self, curr_s, curr_city_time):
        ...
        s_grid[:, self.n_valid_grids * 2 : self.n_valid_grids * 2 + T] = np.stack(
            [time_one_hot] * self.n_valid_grids
        )
