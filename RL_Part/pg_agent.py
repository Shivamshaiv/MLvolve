...
import marl
from marl.agent import (ContinuousDQNAgent, DQNAgent, MATrainable, QTableAgent,
                        TrainableAgent)
...
class PHCAgent(PGAgent):
    ...
    def update_actor(self, batch):
        ...
        delta_sa = torch.Tensor(
            [min(self.policy.model(obs, a), self.delta / (n_a - 1)) for a in range(n_a)]
        )
        delta_sa[max_a] = 0.0
        for a in range(n_a):
            self.policy.model.value[obs, a] = max(
                self.policy.model.value[obs, a] - delta_sa[a], 0
            )
...
class DDPGAgent(PGAgent):
    ...
    def __init__(
        ...
        self.critic = ContinuousDQNAgent(
            qmodel=critic_model,
            actor_policy=policy_for_critic,
            observation_space=observation_space,
            action_space=action_space,
            experience=self.experience,
            gamma=self.gamma,
