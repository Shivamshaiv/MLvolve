...
from . import MATrainable, TrainableAgent
...
class QAgent(TrainableAgent):
    ...
    def __init__(
        ...
        assert isinstance(
            self.experience, ReplayMemory
        ), "This version only support Replay Memory as experience"
    ...
    def update_model(self, t):
        ...
        if isinstance(self, MATrainable):
            ...
            batch = self.mas.experience.get_transition(
...
class QTableAgent(QAgent):
    ...
    def update_q(self, curr_value, target_value, batch):
        self.policy.Q.q_table[
            batch.observation, batch.action
        ] = curr_value + self.lr * (target_value - curr_value)
    ...
    def target(self, Q, batch):
        ...
        not_dones = torch.from_numpy(1.0 - batch.done_flag).float()
...
class MinimaxQAgent(QAgent, MATrainable):
    ...
    def __init__(
        ...
        QAgent.__init__(
            ...
            qmodel=MultiQTable(
                ...
                [gymSpace2dim(my_action_space), gymSpace2dim(other_action_space)],
            ),
            observation_space=observation_space,
    ...
    def update_q(self, curr_value, target_value, batch):
        ...
        self.policy.Q.q_table[
            batch.observation[0][self.index],
            batch.action[0][self.index],
            batch.action[0][1 - self.index],
        ] = curr_value + self.lr * (target_value - curr_value)
    ...
    def target(self, Q, joint_batch):
        ...
        not_dones = (
            torch.from_numpy(1.0 - joint_batch.done_flag).squeeze(0)[self.index].float()
        )
        target_value = rew + not_dones * self.gamma * next_value
        return target_value

    def value(self, observation, action):
        return self.policy.Q.q_table[
            observation[0][self.index], action[0][self.index], action[0][1 - self.index]
        ]
...
class ContinuousDQNAgent(DQNAgent):
    ...
    def target(self, Q, batch):
        ...
        not_dones = torch.from_numpy(1.0 - batch.done_flag).float()
        target_value = (
            rew + not_dones * self.gamma * next_state_action_values
        ).unsqueeze(1)
        return target_value.detach()
