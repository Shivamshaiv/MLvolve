...
class ReplayMemory(Experience):
    def __init__(self, capacity, transition_type="FFTransition"):
        ...
        self.transition = namedtuple(
            "Transition", field_names=transition[transition_type]
        )

    def push(self, *transition):
        assert len(self.transition._fields) == len(
            transition
    ...
    def __repr__(self):
        return "ReplayMemory<{}>({}/{})".format(
            self.transition_type, len(self), self.capacity
        )
...
class PrioritizedReplayMemory(Experience):
    ...
    def __init__(
        self, capacity, alpha=0.6, beta=0.4, eps=1e-6, transition_type="FFTransition"
    ):
    ...
    def push(self, error, transition):
        assert len(self.transition._fields) == len(
            transition._fields
        ), "Invalid number of transition values : {} given instead of {} required".format(
            len(transition._fields), len(self.transition._fields)
        )
    ...
    def __repr__(self):
        return "PrioritizedReplayMemory<{}>({}/{})".format(
            self.transition_type, len(self.tree), self.tree.capacity
        )
